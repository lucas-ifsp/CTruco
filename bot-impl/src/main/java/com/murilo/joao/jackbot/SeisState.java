...
public class SeisState implements RaiseResponseStatePattern {
    ...
    public int handleFirstRound(GameIntel intel) {
        ...
        if (bot.hasSuperHand(intel)) {
            // Se tiver uma mão super, aceita o truco
            return 0; // Aceita
        } if (bot.hasGoodHand(intel) || bot.hasStrongHand(intel)) {
            // Se tiver uma mão boa ou forte, desiste do truco
            return -1; // Quit
        } if (bot.hasOkHand(intel) || bot.hasBadHand(intel) || bot.hasTheWorstHand(intel)) {
            // Se tiver uma mão ok, ruim ou péssima, desiste do truco
            return -1; // Quit
        } if (bot.hasSuperPlusHand(intel) || bot.hasExtremeHand(intel) || bot.hasInvincibleHand(intel)) {
            // Se tiver uma mão super plus, extrema ou invencível, sobe o valor para nove
            return 1; // Re-raise
        } else {
