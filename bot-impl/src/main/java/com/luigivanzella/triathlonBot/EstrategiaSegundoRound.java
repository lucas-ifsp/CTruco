...
public class EstrategiaSegundoRound extends AbstractEstrategiaRound {
    ...
    public boolean decideIfRaises(GameIntel intel) {
        ...
        if(getlastRoundResult(intel).equals(GameIntel.RoundResult.WON)){
            if(getNumberOfManilhas(intel) > 0 || getAverageCardValue(intel) > 8) return true;
            else return false;

        }else{
            if (getNumberOfManilhas(intel) > 1) return true;
            if (getNumberOfManilhas(intel) > 0 && getAverageCardValue(intel) > 8) return true;
            if(getAverageCardValue(intel) > 8.5) return true;
            else return false;
        }
    }
    ...
    public int getRaiseResponse(GameIntel intel) {
        ...
        if(getlastRoundResult(intel).equals(GameIntel.RoundResult.WON)){
            if(getNumberOfManilhas(intel) > 0 || getAverageCardValue(intel) > 8.5) return 1;
            if(getAverageCardValue(intel) > 8) return 0;
            else return -1;

        }else{
            if (getNumberOfManilhas(intel) > 1) return 1;
            if (getNumberOfManilhas(intel) > 0 && getAverageCardValue(intel) > 8) return 0;
            if(getAverageCardValue(intel) > 8.5) return 0;
            else return -1;
        }
    }
